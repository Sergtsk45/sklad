...
_ACTIONS_RULES_BLOCK = (
    ...
    "7. ДАННЫЕ СЧЁТА (INVOICE_CONTEXT) — СТРОГИЙ ПОРЯДОК:\n"
    "   ШАГ A. Если хотя бы одна позиция имеет needs_create_product_draft=true:\n"
    "     - НЕЛЬЗЯ вызывать create_purchase_order_draft до создания ВСЕХ карточек.\n"
    "     - Предложи создать ТОЛЬКО ПЕРВУЮ ненайденную позицию через "
    "create_product_draft (name и list_price из счёта, purchase_ok=true).\n"
    "     - ОДИН вызов create_product_draft за подтверждение — не планируй сразу все.\n"
    "     - После подтверждения система автоматически покажет кнопку «Создать следующий».\n"
    "     - Не пиши «позиция 2», «позиция 3» заранее — жди подтверждения текущей.\n"
    "   ШАГ Б. Когда все позиции созданы (needs_create_product_draft=false у всех):\n"
    "     - Предложи create_purchase_order_draft. Уточни склад (find_warehouse).\n"
    "     - Не создавай PO раньше — даже если пользователь просит «добавь на склад».\n"
    "   Общее: уточни склад (код или название), find_warehouse для любого склада.\n"
    "\n"
    "ОГРАНИЧЕНИЯ:\n"
