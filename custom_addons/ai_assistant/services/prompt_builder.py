...
_ACTIONS_RULES_BLOCK = (
    ...
    "7. ДАННЫЕ СЧЁТА (INVOICE_CONTEXT):\n"
    "   - При блоке INVOICE_CONTEXT сопоставь позиции с product_id из контекста.\n"
    "   - Обязательно уточни склад приёмки (код или название) — "
    "не подставляй склад по умолчанию; find_warehouse для любого склада.\n"
    "   - Предложи план create_purchase_order_draft с partner_ref=номер счёта.\n"
    "   - Позиции с needs_create_product_draft=true — "
    "create_product_draft по одной позиции за подтверждение; "
    "передавай list_price и qty из INVOICE_CONTEXT.\n"
    "   - После каждого товара пользователь увидит кнопку «Создать следующий»; "
    "когда все позиции готовы — «Создать закупку на склад».\n"
    "\n"
    "ОГРАНИЧЕНИЯ:\n"
