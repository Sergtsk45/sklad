...
class TestObr012ConfirmIssue(TransactionCase):
    ...
    def setUp(self):
        ...
        self.product = self.env['product.product'].create({
            ...
            'type': 'consu',
            'is_storable': True,  # В Odoo 19: consu + is_storable=True = складской товар
        })
    ...
    def test_sync_updates_qty_issued_from_move(self):
        """_sync_qty_issued_to_request_lines обновляет qty_issued из move.quantity."""
        picking = self._create_picking()
        move = self.line.issue_move_id
        move.write({'quantity': 6.0})
    ...
    def test_line_state_fully_supplied_after_full_sync(self):
        """После полной синхронизации line_state == 'fully_supplied'."""
        picking = self._create_picking()
        move = self.line.issue_move_id
        move.write({'quantity': 6.0})
        picking._sync_qty_issued_to_request_lines()
    ...
    def test_validate_picking_updates_qty_issued(self):
        ...
        # Set done qty = demanded qty on move lines
    ...
    def test_validate_picking_line_state_fully_supplied(self):
        """После подтверждения выдачи line_state строки == 'fully_supplied'."""
        self._add_stock()
        picking = self._create_picking()
        picking.action_confirm()
        picking.action_assign()
    ...
    def test_notify_posted_when_all_supplied(self):
        ...
        self.line.write({'qty_issued': 6.0})
    ...
    def test_notify_after_full_validate(self):
        """После полного подтверждения выдачи уведомление публикуется в chatter."""
        self._add_stock()
        picking = self._create_picking()
        picking.action_confirm()
        picking.action_assign()
