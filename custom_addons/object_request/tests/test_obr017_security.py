...
def _make_user(env, name, login, group):
    """Создать пользователя с указанной группой модуля."""
...
class TestACLSupplyManager(TransactionCase):
    ...
    def test_supply_can_delete_line(self):
        ...
        self.assertFalse(
            self.env["object.request.line"].browse(line_id).exists()
        )
...
class TestACLStorekeeper(TransactionCase):
    ...
    Кладовщик:
    - читает и редактирует только документы 'in_progress'
