...
class ObjectRequestLineStock(models.Model):
    ...
    line_id = fields.Many2one(
        'object.request.line', string='Строка требования',
    ...
    company_id = fields.Many2one(
        'res.company', string='Компания', related='line_id.company_id', store=True,
    )
    qty_on_hand = fields.Float(
        string='Доступно на складе', digits='Product Unit of Measure',
    )
    qty_to_issue = fields.Float(
        string='К выдаче со склада', digits='Product Unit of Measure',
    )
        ...
        for line in self.mapped('line_id'):
            ...
            if planned > allowed + 0.00001:
                raise ValidationError(
                    'Сумма к выдаче по складам не может превышать остаток к обеспечению.'
                )
