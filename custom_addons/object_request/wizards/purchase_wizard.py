...
class ObjectRequestPurchaseWizard(models.TransientModel):
    ...
    create_draft_only = fields.Boolean(
        string='Только черновики', default=True,
    )
    comment = fields.Text(string='Комментарий')
    line_count = fields.Integer(
        compute='_compute_counts', string='Строк к закупке',
    )
    lines_without_vendor_count = fields.Integer(
        compute='_compute_counts', string='Строк без поставщика',
    )

    @api.depends('line_ids', 'line_ids.preferred_vendor_id', 'line_ids.qty_to_buy')  # noqa: E501
    def _compute_counts(self):
        for wiz in self:
            wiz.line_count = len(wiz.line_ids)
            wiz.lines_without_vendor_count = sum(
                1 for ln in wiz.line_ids
                if ln.qty_to_buy > 0
                and ln.product_id
