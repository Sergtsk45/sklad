...
class ObjectRequestIssuePreviewWizard(models.TransientModel):
    ...
    group_ids = fields.One2many(
        'object.request.issue.preview.group', 'wizard_id',
    ...
    def default_get(self, fields_list):
        ...
        for stock in stock_ids:
            groups_by_warehouse.setdefault(stock.warehouse_id, self.env[
        ...
        for warehouse, warehouse_stock_ids in groups_by_warehouse.items():
            group_vals.append((0, 0, {
                ...
                'destination_location_id': customer_loc.id if customer_loc else False,
...
class ObjectRequestIssuePreviewGroup(models.TransientModel):
    ...
    warehouse_id = fields.Many2one(
        'stock.warehouse', string='Склад', required=True, readonly=True,
    )
    picking_type_id = fields.Many2one(
        'stock.picking.type', string='Тип операции', required=True,
    )
    source_location_id = fields.Many2one(
        'stock.location', string='Откуда', required=True,
    )
    destination_location_id = fields.Many2one(
        'stock.location', string='Куда', required=True,
    )
    scheduled_date = fields.Datetime(
        string='Дата выдачи', required=True,
    ...
    def _onchange_picking_type_id(self):
        for group in self:
            if group.picking_type_id.default_location_src_id:
                group.source_location_id = group.picking_type_id.default_location_src_id
    ...
    def _create_picking(self):
        self.ensure_one()
        stock_lines = self.stock_line_ids.filtered(lambda stock: stock.qty_to_issue > 0)
        if not stock_lines:
            raise UserError(f'Нет строк к выдаче по складу {self.warehouse_id.name}.')
        picking = self.env['stock.picking'].create({
            ...
            'object_request_project_id': self.wizard_id.request_id.project_id.id,
        })
        move_vals = []
