...
class ObjectRequestStockCheckWizard(models.TransientModel):
    ...
    line_ids = fields.One2many(
        ...
        "wizard_id",
        string="Позиции с наличием",
        readonly=True,
    )
    lines_with_stock_count = fields.Integer(
        string="Позиций с наличием",
        compute="_compute_counts",
    )
    lines_total_count = fields.Integer(
        string="Всего позиций",
        compute="_compute_counts",
    )
    warehouse_names = fields.Char(
        string="Проверено по складам",
        compute="_compute_warehouse_names",
    )
    ...
    def _compute_counts(self):
        for wiz in self:
            ...
            wiz.lines_total_count = len(
                wiz.request_id.line_ids.filtered(
    ...
    def default_get(self, fields_list):
        ...
        lines_with_stock = request.line_ids.filtered(
            ...
            and not ln.is_cancelled
            and ln.stock_qty_on_hand > 0
        )
        res["line_ids"] = [
            (
                0,
...
class ObjectRequestStockCheckWizardLine(models.TransientModel):
    ...
    product_id = fields.Many2one(
        "product.product", string="Товар", readonly=True
    )
