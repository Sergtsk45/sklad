...
- **Статус**: ⏳ В процессе (шаблон создан, ожидает ручного прогона)
- **Приоритет**: Высокий
...
- **Шаги выполнения**:
  - [x] Создать шаблон `docs/pilot_results_v2.md` с 15 сценариями и инструкциями по прогону
  - [ ] **Текстовые сценарии** (knowledge base v2) — T-01..T-10:
    - [ ] T-01: «Как создать новый склад?» — должен сказать «Новое», не «Создать»
    - [ ] T-02: «Как добавить товар?» — пошаговая инструкция из RST docs
    - [ ] T-03: «Как создать заказ на продажу?» — корректные термины
    - [ ] T-04: «Как создать контрагента?»
    - [ ] T-05: «Как провести инвентаризацию?» — сложный сценарий
    - [ ] T-06: «Как настроить маршруты пополнения?» — akaidoo context
    - [ ] T-07: Переключение между модулями → контекст обновляется
    - [ ] T-08: Вопрос не по теме → корректный отказ
    - [ ] T-09: OpenRouter недоступен → fallback-сообщение
    - [ ] T-10: Вопрос по object_request → akaidoo context кастомного модуля
  - [ ] **Vision-сценарии** (скриншот) — V-01..V-05:
    - [ ] V-01: «Что у меня на экране?» (список товаров)
    - [ ] V-02: «У меня на экране ошибка, помоги» (validation error)
  ...
  - [ ] Заполнить результаты в `docs/pilot_results_v2.md` (ответы, оценки, замечания)
...
- **Шаги выполнения**:
  - [x] Создать `docs/audit_term_mapping_checklist.md` — структурированный чеклист 175 маппингов по 15 секциям (Склад, Продажи, Закупки, CRM, Контакты, Настройки, общие элементы)
  - [ ] **Склад** (stock) — 5 экранов по чеклисту
  - [ ] **Продажи** (sale) — 2 экрана по чеклисту
  - [ ] **Закупки** (purchase) — 2 экрана по чеклисту
  - [ ] **CRM** (crm) — 2 экрана по чеклисту
  - [ ] **Контакты** (contacts) — 2 экрана по чеклисту
  - [ ] **Настройки** (settings) — 1 экран по чеклисту
  - [ ] Заполнить колонку «Реальный RU» и «Статус» в `audit_term_mapping_checklist.md`
  - [ ] Обновить `term_mapping.json` с результатами (найденные расхождения)
  - [ ] Перезапустить `scripts/update_knowledge_v2.sh --skip-akaidoo` для пересборки docs
- **Критерий готовности**: Все 175 маппингов проверены; term_mapping обновлён; knowledge base пересобрана.
...
- [x] При триггерных фразах — захват скриншота и ответ на основе изображения (AIA-020..023 ✅)
- [x] Vision-запросы уходят на мощную модель (AIA-024: контроллер выбирает vision_model из настроек ✅)
- [x] Администратор может менять обе модели в Settings (AIA-017..019 ✅)
- [x] Скриншоты не логируются, не сохраняются: `saveHistory()` стрипает base64, `_parse_screenshot()` не логирует данные (AIA-022..024 ✅)
- [x] Rate limit на vision-запросы работает: 5 req/min per user, `_VISION_RATE` dict (AIA-026 ✅)
- [ ] response_guard совместим с ответами vision-модели (не проверялось — нужен ручной тест)
- [x] Knowledge base обновляется одним скриптом (`scripts/update_knowledge_v2.sh`)
- [ ] Все unit-тесты проходят (тесты написаны AIA-027/028; прогон — после запуска стека без конфликта порта 8069)
- [ ] ≥12 из 15 пилотных сценариев оценены как ✅ (шаблон: `docs/pilot_results_v2.md`; ожидает ручного прогона AIA-029)
- [ ] Модуль обновляется без ошибок (`-u ai_assistant`) — подтвердить прогоном
