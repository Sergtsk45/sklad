...
- Поиск поставщика по ИНН или названию.
...
- Пошаговый сценарий: сначала карточки товаров → затем заказ поставщику.
- Подсказки-кнопки (chips) для быстрых действий в диалоге.

## Чего ассистент не делает (ограничения безопасности)

- Не подтверждает заказы поставщику (**Confirm PO**).
- Не проводит приходы и перемещения (**Validate**).
- Не создаёт счета поставщика и платежи (оплата — в **1С**).
...
| `find_partner` | `res.partner` | Поставщик по ИНН или названию |
...
### Алгоритм внутреннего перемещения (с базы)

1. Проверить остатки на базе (`search_stock_quants`).
2. Определить `int_type_id` объекта и `lot_stock_id` назначения.
3. Создать черновик picking: локация базы → склад объекта.
4. Указать `origin` = номер OR.
5. **Ваш шаг** — Validate в UI.
...
2. Выберите PDF или Excel (до 5 МБ).
3. Ассистент извлекает: поставщик (ИНН, название), номер счёта, дату, позиции, цены, итог.

### Пошаговый workflow (важно)

Ассистент следует **жёсткому порядку**:

1. **Шаг A — товары:** для каждой позиции без совпадения в номенклатуре — карточка `create_product_draft` (с ценой из счёта). Подтверждайте по одной или пакетно через chips.
2. **Шаг B — заказ:** когда все позиции сопоставлены — план PO. Ассистент **уточнит склад/объект** (не подставляет склад по умолчанию).
3. **Шаг C — ваши действия в UI:** Confirm PO → Validate Receipt.

### Связь с 1С

В Odoo фиксируется только **ссылка на внешний счёт**:

- в PO: поле **«Ссылка поставщика»** (`partner_ref`) = номер счёта (например `УТ-1132`);
- факт оплаты в Odoo **не отражается** — бухгалтерия работает в 1С.

### Пересчёт труб (кг / тонны → метры)

Поставщики часто выставляют счета в **кг** или **тоннах**, а склад учитывает **метры**.

Сейчас ассистент **подсказывает формулу пересчёта**, но записывает в строки уже **согласованные метры**, которые вы подтверждаете:
...
| П4 | **Автосоздание/обновление поставщика из счёта** | Не искать ИНН вручную перед PO | Расширение `find_partner` + write-tool `create_partner_draft` с подтверждением |
...
*Черновик подготовлен на основе состояния репозитория на 2026-06-11. Перед публикацией для пользователей рекомендуется согласование с отделом снабжения и ревью администратором Odoo.*
