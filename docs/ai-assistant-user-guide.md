...
| `create_partner_draft` | карточку поставщика | Только новый контрагент, ИНН обязателен, банковские реквизиты не переносятся |
...
1. **Шаг A — поставщик:** если ИНН поставщика из счёта не найден в Odoo, ассистент покажет chip «Создать поставщика…» и карточку `create_partner_draft`. Создание выполняется только кнопкой **Подтвердить**.
