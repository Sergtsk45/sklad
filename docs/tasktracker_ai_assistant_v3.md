...
- **Шаги выполнения**:
  - [x] Для `create_purchase_order_draft` — расширенный `next_hint`/`steps`: «Откройте черновик → **Подтвердить** → откройте **Приход** → **Проверить/Провести**»
  - [x] Путь меню + напоминание про оплату/счёт в 1С
  - [x] Рендер шагов в OWL ResultCard (нумерованный список `<ol>`)
  - [x] Тест: карточка PO содержит шаги Confirm + Validate
...
- **Шаги выполнения**:
  - [x] Фикстура НФ-504 (нормализованный dict, без реального файла/HTTP)
  - [x] Прогон: парс → сопоставление → создание товара → PO draft
  - [x] Ассерты: `state='draft'`, 14 строк, сумма 72 096,22, заметка в chatter
  - [x] Обновить `changelog.md`, статус задач в трекере
