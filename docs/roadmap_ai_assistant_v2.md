...
**Дата:** 2026-03-21  
**Статус:** Проект (V2-1 и большая часть V2-2 в коде; V2-3 vision — в работе)  
**Базируется на:** существующий модуль `custom_addons/ai_assistant/` (AIA-001 — AIA-011 выполнены)

**Фактическое выполнение (аудит 2026-03-21):**

| Этап | Состояние |
|------|-----------|
| V2-1 Knowledge (RST, term_mapping, `KnowledgeProviderV2`, `prompt_builder` v2, скрипты) | ✅ в репозитории |
| V2-2 Две модели в настройках + `model_override` в `OpenRouterClient` | ⚠️ UI/клиент готовы; переключение vision из `chat()` — после приёма скриншота |
| V2-3 Screenshot + vision pipeline | ❌ не начато (`html2canvas`, триггеры, payload, `_parse_screenshot`, multimodal в `build_messages`) |
| V2-4 Тесты v2, пилот, аудит UI | ❌ / частично (нет `test_knowledge_provider_v2.py`, нет `pilot_results_v2.md`) |

Детализация по ID задач — в `docs/tasktracker_ai_assistant_v2.md`.
...
_Чеклист синхронизирован с `docs/tasktracker_ai_assistant_v2.md` (аудит 2026-03-21)._

- [x] Ассистент отвечает на основе RST-документации Odoo 19, не на ручных JSON
- [x] Термины кнопок/меню — через `term_mapping.json` + промпт (живой UI — отдельный аудит AIA-030)
- [x] term_mapping.json содержит ≥80 верифицированных маппингов
- [ ] При триггерных фразах — захват скриншота и ответ на основе изображения
- [ ] Vision-запросы уходят на мощную модель, текстовые — на дешёвую (инфраструктура клиента есть)
...
- [x] Knowledge base обновляется скриптом за одну команду
