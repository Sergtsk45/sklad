...
12. После нажатия **«Выполнить»** ассистент выполняет только пункты с ответом «Да» в безопасном порядке: создать/подтвердить закупку → привязать счёт → провести приёмку.
...
- **Нет на «Привязать счёт?»**: ассистент сохраняет решение и после финального запуска не создаёт `ir.attachment`.
...
flowchart TD
    ...
    CreatePO --> MaybeAttach{attach_invoice?}
    MaybeAttach -->|Да| AttachInvoice[Создать ir.attachment]
    MaybeAttach -->|Нет| MaybeReceive
    AttachInvoice --> MaybeReceive{receive_picking?}
    MaybeReceive -->|Да| ValidateReceipt[button_validate incoming picking]
    MaybeReceive -->|Нет| Done
    ValidateReceipt --> Done
```
...
- **Шаги выполнения:**
 ...
 - [x] Не создавать `account.move`; в рамках текущего процесса счёт ведётся как PDF-вложение к PO.
- **Зависимости:** AIA-061.
- **DoD:** PDF счёта появляется во вложениях PO только после ответа «Да» и финального нажатия **«Выполнить»**.
