## [2026-09-11] — TD-015: комментарий в списке требований на комплектацию

### Добавлено
- В общем списке требований на комплектацию колонка **«Комментарий»**
  (поле шапки `object.request.comment`, `optional="show"`).
- Поиск списка находит документ по комментарию: отдельное поле в search
  view и `filter_domain` общей строки (номер **или** комментарий).
  Комментарий строки (`line.comment`) в этот поиск не входит.
- Тесты: `test_td015_request_comment_list.py` — 4 post-tests, 0 failed.
- Версия: `object_request` `19.0.1.10.31`.
- Закрыт [`TD-015`](technical-debt.md#td-015-колонка-комментарий-и-поиск-по-нему-в-списке-требований-на-комплектацию).
- Prod deploy `6a6fbd4`: backup
  `/opt/project_odoo/backups/or-td015-comment-list-20260911-070325/`,
  `-u object_request` → `19.0.1.10.31`, restart; health `pass`, `/web/login`
  HTTP 200.

## [2026-09-09] — Prod SMTP: extra_hosts для smtp.mail.ru

### Исправлено
- Контейнер Odoo на VPS не резолвил `smtp.mail.ru` (`Temporary failure in
  name resolution`), заявки P00126 не уходили ни поставщику, ни на
  `675001@mail.ru`. В `docker-compose.yml` у сервиса `odoo` добавлены
...
### Исправлено
- Композер вырезал подпись из тела, а layout для admin её не возвращал.
...
### Исправлено
- «Отправить запрос» слал два разных HTML: поставщик в группе `portal_customer`
...
## [2026-09-10] — Каталог: НФ-1373 (Татаринов, клапаны/фасон/прокладки)

### Изменено
- Счёт № 1373 / НФ-1373 от 10.09.2026 (ИП Татаринов, id 12): все 12 позиций
  уже были в каталоге; обновлены vendor-алиасы и `standard_price`.
- Нормализованы имена: **72** (клапан Ду25), **697** (клапан Ду50, бренд
  Гранлок убран из `name`), **423** (паронит Ду32), **470** (муфта Ду25 + ГОСТ).
- Карточки: 72, 697, 96, 90, 79, 81, 423, 699, 427, 616, 470, 477.
  Дубль тройника 89×3,5 (**1125**) не сливали — мастер **81**.

## [2026-09-02] — Каталог: счёт № 3315 (ЭТМ, ЗРК VTM)

### Изменено
- Поставщик АО «Энерготехномаш» (ЭТМ) **17** (ИНН 0323085570) — уже был;
  в comment добавлен КПП 032601001.
- Карточки **604–608** и vendor-алиасы **516–520** (артикулы 23712, 23711,
  23805, 23804, 23904) совпали со счётом № 3315 от 25.05.2026; цены
  подтверждены, в chatter — ссылка на счёт. Новых карточек не создавалось.

## [2026-09-02] — Каталог: ЗРК по счёту № 114 (ДВ Партнёр)

### Изменено
- Сверка счёта № 114 от 25.08.2026: уникальные типоразмеры уже были
  **1007** (DN20 Kv4,0), **1008** (DN32 Kv16), **1009** (DN50 Kv25) с vendor
  ООО «ДВ Партнёр» (id 60). Алиасы **950–952** подтверждены/обновлены,
  `standard_price` = цена счёта; в chatter — ссылка на № 114.
- В счёте строки 1 и 2 с одинаковым обозначением DN20, цены 47 500 и 61 650;
  в vendor оставлена **61 650** ₽ (строка 2, 2 шт).

## [2026-09-02] — Каталог: ШВВП 2×0,75 (УТ-2198, УТ-46345)

### Изменено
- На **642** «Провод ШВВП (ПуГВВ) 2х0,75 б (100,200)» добавлены vendor-алиасы:
  - ООО «Электро Центр» (УТ-2198): `NKZ–A0U1Т0N8KXZN–2AS00U.715ТL08XN2S0.75L`, 37,00 ₽/м (id **1346**);
  - ООО «ТД «СВЕТОТЕХНИКА»» (УТ-46345): `SQ0120-0010`, 36,00 ₽/м (id **1347**).

## [2026-08-30] — Подпись RFQ не теряется в копии письма

### Исправлено
- «С уважением,» больше не внутри QWeb `<t t-set>`: санитайзер Odoo вырезал
  тег вместе с текстом, и подписи не было ни у поставщика, ни в копии на
  `675001@mail.ru`. `object_request` `19.0.1.10.28`.

## [2026-08-30] — Подпись RFQ: «С уважением,»
...
### Добавлено
- В `ai_assistant` добавлен детерминированный сервис пересчёта труб из кг/т/хлыстов в метры с человекочитаемой формулой и ошибками при отсутствии `kg_per_meter` или длины хлыста.
...
### Добавлено
- Ручной autocomplete товара в OR находит карточки по тегам шаблона
  (`product_tag_ids`) и варианта (`additional_product_tag_ids`). Поиск
  включается только явным OR-контекстом и не меняет глобальные товарные поля
  или автоматическое AI-сопоставление.
- Для нескольких слов действует AND-семантика; результаты по имени, коду и
  торговому названию остаются выше tag-only позиций. Соблюдаются caller-domain,
  дедупликация, `limit=0` и обычные ограничения доступа.
- Vendor-filter применяется и к tag-only результатам, поэтому тег не выводит
  товар за пределами каталога выбранного поставщика.
- Контекст включён в форме требования, автономном списке строк, массовом
  назначении товара и ручном выборе в предпросмотре импорта.
...
### Исправлено
- Поле `object.request.line.name_raw` доступно для изменения только в
  черновике требования. В статусе «В работе» и финальных статусах запрет
  действует в форме, автономном списке строк и на сервере через `write()`.
- Добавлены тесты разрешённой правки в черновике, серверного запрета вне
  черновика, записи прежнего значения и `readonly` в XML-представлениях.
- Проверки: focused `TestStateMachine` — 30 post-tests; полный
  `/object_request` — 552 post-tests, 0 failed, 0 errors.
- Версия: `object_request` `19.0.1.10.22`.

## [2026-08-28] — Исправлена ошибка компиляции стилей backend

### Исправлено
- `object_request.scss`: CSS `min(rem, vw)` в `.o_or_full_dropdown` больше
  не ломает libsass («Incompatible units: vw and rem»). Из‑за этого Odoo
  показывал «Ошибка стиля. Компиляция стилей не удалась» на всей базе.
- Версия: `object_request` `19.0.1.10.21`.

## [2026-08-31] — Каталог: УТ-129 (Цыплухин, электроды)

### Добавлено
- **1284** Электроды сварочные J422, Ø3,2 мм — vendor ИП Цыплухин
  «Золотой мост», 144 ₽/кг (счёт УТ-129 от 26.08.2026).

### Изменено
- **827** ОК 46.00 Ø3,0×350 — vendor Цыплухин обновлён: 427 ₽/кг
  (коды 4600303АМ0/АЕ0, ЭСАБ/СВЭЛ).
- **1265** УОНИ-13/55 Ø3 мм — vendor Цыплухин 175 ₽/кг (СЗСМ).

## [2026-08-31] — Каталог: маска сварщика К-101 (УТ-133)

### Добавлено
- **1283** Маска сварщика хамелеон К-101, чёрная — vendor ИП Цыплухин
  (счёт УТ-133 от 31.08.2026): КЕДР PRIME PROMO, 2 288 ₽/шт (без НДС).

## [2026-08-31] — Каталог: слияние сгонов Ду25

### Изменено
- Мастер **934** «Сгон стальной Ду25» (длина по ГОСТ, без L в имени).
- Перенесены vendor: Татаринов (с **456**), ТД ЦС `360025-110` (с **1282**);
  Шалаева уже был на 934.
- Строка OR/2026/08/0068 #742 переведена с 456 → **934**.
- Архив: **456**, **1282**.

## [2026-08-31] — Каталог: УТБФ0007363 (ТД Центр Снабжения)

### Добавлено
- **1282** Сгон стальной Ду25, L110 мм — vendor ТД ЦС `360025-110`, 54,01 ₽/шт.

### Изменено
- Vendor ООО ТД Центр Снабжения по счёту УТБФ0007363 от 28.08.2026:
  краны **629**/**1049**, тройник **862**, контргайка **483**, муфта **470**,
  резьба **970**, бочонок **611**, муфта PPR **833**, фильтр **508**.
- Нормализованы имена: **483** «Контргайка чугунная Ду25»,
  **508** «Фильтр сетчатый латунный Ду25».

## [2026-08-28] — Каталог: ЦБ-1682 (РИКОЛЬ, кровельные материалы)

### Добавлено
- Теги `product.tag`: кровельный, гидроизоляция, рулонный, битумный,
  подкладочный, праймер.
- **1279** Ковер подкладочный битумно-полимерный, 15 м² — vendor ООО «РИКОЛЬ»
  3 764,20 ₽/шт (рулон); теги: кровельный, гидроизоляция, рулонный, битумный,
  подкладочный.
- **1280** Ковер кровельный битумно-полимерный, посыпка сланец, серый, 10 м² —
  vendor РИКОЛЬ 2 909,50 ₽/шт; теги: кровельный, гидроизоляция, рулонный,
  битумный.
- **1281** Праймер битумный, 18 кг — vendor РИКОЛЬ 4 941,20 ₽/шт; теги:
  праймер, битумный, гидроизоляция.

### Изменено
- **TD-014** в `docs/technical-debt.md`: поиск товара в OR по тегам
  `product.tag` (сейчас теги не участвуют в `name_search`).

## [2026-08-27] — Правило: заметки UPG-* при правках ядра

### Добавлено
- `.cursor/rules/odoo-upgrade-notes.mdc` (alwaysApply): при переопределении
  ядра Odoo сразу писать пункт в `docs/odoo-upgrade-notes.md`. То же в
  `CLAUDE.md` и `docs/rulesworkproject.md`.

## [2026-08-27] — Заметки для перехода на Odoo 20

### Добавлено
- [`docs/odoo-upgrade-notes.md`](odoo-upgrade-notes.md): реестр правок ядра
  на проверку при апгрейде. Первый пункт **UPG-001** — письмо RFQ без кнопки
  портала / P00xxx / срока (ссылки из `docs/project.md` и `docs/deploy.md`).

## [2026-08-27] — Заявка поставщику без кнопки портала

### Изменено
- Письмо «Отправить запрос» (RFQ): убраны кнопка «Посмотреть предложение»
  (`/my/purchase/…` с токеном), номер заказа (P00xxx) и срок в шапке.
  Остаются тема «Заявка на счёт», таблица позиций и подпись ТСК.
- Подтверждённый заказ («Отправить заказ») без изменений.
- Версия: `object_request` `19.0.1.10.20`.
- Prod deploy `7e2b93f`: backup
  `/opt/project_odoo/backups/or-rfq-no-portal-button-20260827-014310/`,
  `-u object_request` → `19.0.1.10.20`, restart; `/web/login` HTTP 200.

## [2026-08-26] — Каталог: клемма 2,5 мм² серая отдельно от синей

### Изменено
- **940** переименована: «Клемма винтовая проходная 2,5 мм², серая»
  (отдельно от **1253** синяя и **1254** PE).

## [2026-08-26] — Каталог: УТ-2117 / Н2544 / УТ-44733 (электрика)

### Добавлено
- **1269** Выключатель автоматический 1P, 2 А, характеристика C
- **1270** Лампа сигнальная светодиодная жёлтая, IP65
- **1271** Лампа сигнальная двухцветная красная/зелёная, 230 В
- **1272** Шина PE 8×12 мм, 12 групп, на DIN-рейку
- **1273** Шина N 8×12 мм, 12 групп, на DIN-рейку
- **1274** НШВИ 0,75-12
- **1275** Переключатель кулачковый 3 полож., 2NO, возврат-фикс-возврат
- **1276** Переключатель кулачковый 3 полож., 2NO, с фиксацией
- **1277** Реле промежуточное MY-3, 220 В AC
- **1278** ЩРНМ-2 IP54, 500×400×150 мм

### Изменено
- **694** нормализован (был CHINT в имени) → «…1P, 4 А, характеристика C»
- **679** нормализован («810 групп» → «10 групп»)
- Vendor-алиасы по счетам: Электро Центр УТ-2117 (id 39),
  ИП Шалаева Н2544 (id 50), ТД Светотехника УТ-44733 (id 84) —
  59 строк supplierinfo (36 новых, 23 обновления) на существующие
  и новые карточки (автоматы, клеммы, контактор, ЩМП IP66, DIN и т.д.).

## [2026-08-24] — Подпись в заявке поставщику

### Добавлено
- В шаблоне «Заявка на счёт» после таблицы товаров: «С уважением,
  ООО "Теплосервис-Комплект", тел. 8 962 285 85 10».

## [2026-08-24] — TD-013: запрет правки «Наименование» в статусе «В работе»

### Добавлено
- В `docs/technical-debt.md` задача **TD-013**: после перехода требования
  на комплектацию в статус «В работе» колонка «Наименование» (`name_raw`)
  должна быть только для чтения (UI + серверная проверка).

## [2026-08-24] — Копия заявки поставщику на 675001@mail.ru

### Изменено
- Шаблон «Purchase: Request For Quotation» (`Заявка на счёт`): получатели —
  поставщик **и** партнёр компании (почта Теплосервис-Комплект,
  `675001@mail.ru`). В composer «Отправить запрос» копия подставляется сама.
- `object_request` `19.0.1.10.18`: при upgrade шаблон RFQ пишется кодом
  (`noupdate` стандартного шаблона иначе игнорирует XML); плюс копия в
  `_message_get_default_recipients`.

## [2026-08-26] — Каталог: УТ-2600 (ПроМеталл, труба 89 + резка)

### Добавлено
- **1268** Резка трубы 57–89 мм (услуга Services) — vendor ООО «ПроМеталл»,
  40 ₽/шт (счёт УТ-2600 от 26.08.2026).

### Изменено
- **583** Труба э/с 89×3,5 (Ду 80) — vendor ПроМеталл обновлён: 708,80 ₽/м
  (96 ₽/кг с НДС, L12; SI 747).

## [2026-08-25] — Каталог: тройники/переход по ЦБ-16945 (Амурснабсбыт)

### Добавлено
- **1266** Тройник стальной переходной 108×57 — vendor АО «АМУРСНАБСБЫТ»
  ЦБ-00033326, 844,26 ₽/шт.
- **1267** Тройник стальной переходной 108×76 — vendor АО «АМУРСНАБСБЫТ»
  ЦБ-00033327, 811,48 ₽/шт.

### Изменено
- **1056** Тройник стальной переходной 89×3,5–76×3,5 — vendor Амурснабсбыт
  ЦБ-00033325, 598,36 ₽/шт (счёт ЦБ-16945 от 21.08.2026).
- **86** Переход стальной 89×76 (Ду80×65) — vendor Амурснабсбыт
  ЦБ-00004568, 176,23 ₽/шт (ГОСТ 17378-2001).

## [2026-08-24] — Каталог: vendor клапан Ду32 (Татаринов)

### Изменено
- **71** Клапан обратный латунный Поворотный Ду32 ру16 — vendor
  ИП Татаринов, **1 024,00 ₽/шт** (прайс). Поиск в OR с фильтром по
  поставщику находит Ду32.

## [2026-08-24] — Каталог: алиас Момент-88 по ЦБ-17102

### Изменено
- **749** Клей Момент 88 750мл — vendor АО «АМУРСНАБСБЫТ» ЦБ-00057856,
  1 368,85 ₽/шт (счёт ЦБ-17102 от 24.08.2026, арт. 1777022).

## [2026-08-24] — Каталог: счёт №2373 (ТСЦ Амур Сварка, электроды)

### Добавлено
- Карточка **1265** «Электроды сварочные УОНИ-13/55, Ø3 мм» (кг, Расходники):
  vendor ТСЦ Амур Сварка, 175 ₽/кг. Отдельно от **1104** (Ø4 мм, М33).

### Изменено
- **827** ОК 46.00 Ø3,0×350 — vendor ТСЦ Амур Сварка обновлён: 242 ₽/кг
  (счёт №2373 от 24.08.2026).

## [2026-08-21] — Скилл закупки из счёта (складской агент)

### Добавлено
- Скилл `odoo-purchase-from-invoice`: `purchase.order` из счёта через MCP.
  Переносимый (без URL/id инстанса). Копии: `.cursor/skills/odoo-purchase-from-invoice/`
  и `~/.cursor/skills/odoo-purchase-from-invoice/`. Примеры — `examples.md`.

### Изменено
- Склад приёмки **не подставляется по умолчанию**: агент предлагает 3–6
  вариантов и ждёт выбора.
- UoM строки — с карточки (пересчёт шт.↔кг), цены с НДС без задвоения налога,
  PO только draft, пустые черновики того же поставщика не переиспользуются.
- `purchase-from-invoice` оставлен как alias. Правило
  `purchase-from-invoice.mdc` — локальный оверлей (ИНН, форматы номеров).

## [2026-08-21] — Скилл каталога: карточки, алиасы, нормализация имён

### Добавлено
- Скилл `odoo-product-catalog` (персональный `~/.cursor/skills/` и проектный
  `.cursor/skills/`): сверка счёта, нормализация `product.template.name`,
  создание карточек, `product.supplierinfo`, слияние дублей. Переносимый
  (без URL/id инстанса). Примеры имён и RPC — `naming-examples.md`, `examples.md`.
- Правило `odoo-product-catalog.mdc` сокращено до локальных id категорий/UoM.

### Изменено
- `purchase-from-invoice`: шаг товаров отсылает к `odoo-product-catalog`.

## [2026-08-21] — Перемещение по счёту 1156 на Контейнер_5 / С2

### Добавлено
- Внутреннее перемещение [Кнт5/INT/00001](https://skladtsk.duckdns.org/odoo/stock.picking/166)
  (`done`): 16 позиций счёта 1156 (P00103) с `Ос.ск/Полки` на `Кнт5/Наличие/С2`, 186 шт.

## [2026-08-21] — Скилл создания складов Odoo

### Добавлено
- Скилл `odoo-create-warehouse`: склад `stock.warehouse` + опциональные полки
  через MCP. Переносимый (без id/URL инстанса). Копии:
  `.cursor/skills/odoo-create-warehouse/` и `~/.cursor/skills/odoo-create-warehouse/`.

## [2026-08-21] — Склад Контейнер_5 с полками

### Добавлено
- Склад **Контейнер_5** (код `Кнт5`, id 37), приёмка в один шаг, закупки на склад разрешены.
- Полки под `Кнт5/Наличие`: **Н1–Н3**, **С1–С3**, **В1–В3**.

## [2026-08-21] — Каталог: счёт УТ-2526 (ПроМеталл, профиль / лист оц.)

### Добавлено
- **1263** Труба стальная профильная 15×15×1,5 мм, длина 6 м (шт., Трубы).
- **1264** Резка трубы профильной 15–30 мм (услуга Services).

### Изменено
- **872** нормализован: «…40×20×2 мм, длина 6 м», ед.изм. шт.; vendor ПроМеталл
  1 112,40 ₽/шт (из УТ-2526: 103 ₽/кг).
- **962** лист оц. 0,5×1250×2500 — vendor ПроМеталл 145 ₽/кг подтверждён.

## [2026-08-21] — Каталог: счёт УТБФ0006988 (ТД ЦС, ПП наружная / переход)

### Добавлено
- Карточка **1262** «Переход ПП с чугуна на пластик Ду110×124, с манжетой, серый»
  (арт. 23124110M, 229,99 ₽). Отдельно от манжеты **767**.

### Изменено
- Vendor ТД ЦС по УТБФ0006988: тройник **786** (00-00036298), отвод **764**
  (00-00036296) — цены подтверждены.

## [2026-08-20] — Номер счёта в закупке — только из PDF

### Изменено
- `partner_ref` / `origin` в закупке из счёта пишутся **как в документе**, без префикса из имени файла.
- Правило и скилл `purchase-from-invoice`: пример «файл НФ-1156, в PDF № 1156» → `1156`.
- Закупка P00103: `partner_ref` исправлен с `НФ-1156` на `1156`.

## [2026-08-20] — Каталог: жидкая теплоизоляция классик 5 л / 10 л

### Изменено
- Карточка `product.template` 654 переименована в **«Жидкая теплоизоляция классик, 5 л»** (`ru_RU` и `en_US`). Бренд и «НГ» остаются в алиасе поставщика.

### Добавлено
- Карточка **«Жидкая теплоизоляция классик, 10 л»** (id 1261), категория «Теплоизоляция», складской учёт, ед. `шт.`
- Алиасы `product.supplierinfo`: ООО «БРОНЯ-АМУР» — «ЖКТП Броня Классик НГ 5 л, арт. 59» (код 59); ИП Колыбихина — «Жидкая теплоизоляция Классик 10л КОРУНД» (код 0149680). Источники: счёт № 40 от 09.06.2026 и КТУТ-278078 от 15.06.2026.

## [2026-08-19] — OR: полная подпись в выпадашке товара и поставщика (OBR-041)

### Исправлено
- Пункты autocomplete **«Товар»** и **«Поставщик»** в строках требования
  больше не обрезаются: список шире, текст переносится, при наведении
  видна полная строка «наименование + поставщик».
- Версия: `object_request` `19.0.1.10.17`.

## [2026-08-19] — TD-012: необеспеченные позиции в списке требований

### Добавлено
- В `docs/technical-debt.md` задача **TD-012**: в списке требований на
  комплектацию показывать количество необеспеченных позиций (строки не
  `fully_supplied` и не `cancelled`).
