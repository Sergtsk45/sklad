## [2026-08-19] — AI-ассистент: «Привязать счёт?» создаёт vendor bill + PDF

### Изменено
- В сценарии закупки по загруженному счёту ответ **«Да»** на **«Привязать счёт?»**
  создаёт черновик счёта поставщика (`account.move`, `in_invoice`) и прикрепляет
  исходный PDF к счёту (копия остаётся на закупке). Счёт не проводится.
- Если в том же плане есть приёмка, она выполняется **до** bill, чтобы количество
  в счёте совпало с принятым. Без приёмки bill берёт заказанное количество.
- Версия: `ai_assistant` `19.0.1.2.3`.
- Prod deploy `b1cef10`: backup
  `/opt/project_odoo/backups/ai-invoice-vendor-bill-20260819-003121/`,
  `-u ai_assistant` → `19.0.1.2.3`, restart; health `pass`, `/web/login`
  HTTP 200.

### Исправлено
- PDF счёта больше не «теряется» только на PO: его видно в **Бухгалтерия →
  Счета поставщиков** во вложениях/чаттере vendor bill.

## [2026-08-19] — OR: недавние поставщики в выпадающем списке (TD-008 / OBR-040)

### Добавлено
- При фокусе на пустом поле **«Поставщик»** в строках требования в начале
  списка показываются до 8 последних поставщиков компании (из
  `preferred_vendor_id` строк OR). Поиск по имени остаётся обычным.
- Тот же autocomplete в списке строк и в мастере «Назначить поставщика».
- Поиск всех остальных `res.partner` не меняется: срабатывает только с
  context `object_request_recent_vendors`.
...
### Исправлено
- В PDF передаточной ведомости блок «Адрес доставки» заполняется из
