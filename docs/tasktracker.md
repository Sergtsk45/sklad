...
- **Статус**: Реализована; ручная UI-проверка ролей ожидает прогона в браузере
...
- **Проверка**: добавлен регрессионный тест структуры формы в
  `test_obr009_mass_actions.py`; ручная UI-проверка ролей
  (прораб/снабженец, `draft`/`in_progress`/`closed`) остаётся для браузера.
