...
- **Статус**: Готово локально (`19.0.1.10.25`)
- **Описание**: После таблицы в письме «Отправить запрос»:
  «С уважением &lt;имя&gt; ООО "Теплосервис-Комплект"». Имя — закупщик
  заказа (`user_id`).
- **Шаги выполнения**:
  - [x] Шаблон `body_html` + `_setup_rfq_copy_mail_template`.
  - [x] Тесты `or_rfq_copy`.
  - [ ] `-u object_request` на prod.
...
- **Статус**: В работе (ядро TD-002 реализовано 2026-08-29, остаются follow-up по справочнику и заполнению коэффициентов)
