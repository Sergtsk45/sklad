...
- **Статус**: Реализована локально (ICE-001…ICE-008); идёт prod-deploy ICE-009
- **Детальный трекер**: `docs/tasktracker-invoice-paid-calendar-event.md`
- **Описание**: При переходе vendor bill (`account.move`, `move_type=in_invoice`) в `payment_state='paid'` автоматически создавать встречу `calendar.event` на 1 час в **первом свободном часовом слоте** рабочего дня (09:00–16:00 по часовому поясу `move.company_id.partner_id.tz or 'UTC'`, обед 12:00–13:00 исключён; занятость — только `show_as='busy'` в календаре снабженца `buyer_user_id`, горизонт поиска 30 дней с принудительным fallback) на дату `object.request.need_date` связанного требования, с участниками — снабженец и прораб, название `"Оплата {bill.name} — {object_request.name}"`, **без email-рассылки при автосоздании** (`no_mail_to_attendees=True`, дальнейшие ручные правки уведомляют как обычно). Дальше пользователь двигает/растягивает встречу вручную в стандартном календаре Odoo Community.
- **Шаги выполнения**:
 - [x] Живая проверка триггера на `BILL/2026/08/0002` (2026-08-08).
 - [x] Ревью №1/#2 спецификации и сверка с исходниками Odoo.
 - [x] ICE-002…ICE-008: модуль `object_request_calendar` 19.0.1.0.0, 22 post-tests ok локально.
 - [ ] ICE-009: commit/push + install на prod (`-i object_request_calendar`) + ручная проверка.
