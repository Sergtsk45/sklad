## Задача: OR — колонка «Комментарий» и поиск по нему в списке (TD-015)
- **Статус**: Задеплоена на prod 2026-09-11 (`6a6fbd4`, `19.0.1.10.31`)
- **Описание**: В общем списке требований на комплектацию нужна колонка
  с комментарием шапки документа. Поиск в таблице должен находить
  документы и по тексту комментария. В самой форме поле `comment` уже есть.
- **Шаги выполнения**:
  - [x] Колонка `comment` в `view_object_request_list`.
  - [x] Поиск: поле в search view + `filter_domain` общей строки (номер
        **или** комментарий).
  - [x] Тесты XML list/search; не смешивать с `line.comment`.
  - [x] Закрыть TD-015, changelog после внедрения.
  - [x] Push + prod: backup, `-u object_request`, health OK.
- **Зависимости**: `object_request`, [`TD-015`](technical-debt.md#td-015-колонка-комментарий-и-поиск-по-нему-в-списке-требований-на-комплектацию).
...
- **Описание**: HTML-композер вырезает «С уважением, …» из тела, а layout
  ...
  одинаково поставщику и на 675001.
- **Шаги выполнения**:
...
- **Статус**: Задеплоена на prod 2026-09-02 (`c043693`, `19.0.1.10.29`)
...
- **Шаги выполнения**:
  ...
  - [x] Тесты `or_rfq_copy`.
  - [x] Деплой вместе с `19.0.1.10.29`.
- **Зависимости**: `object_request`, `purchase.email_template_edi_purchase`.
