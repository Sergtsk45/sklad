...
- **Статус**: Готово локально (`19.0.1.10.20`)
...
- **Шаги выполнения**:
  ...
  - [x] Тесты `or_rfq_copy`.
  - [ ] `-u object_request` → `19.0.1.10.20` на prod.
...
- **Описание**: После перехода требования на комплектацию в статус
  ...
  можно менять.
- **Шаги выполнения**:
  - [ ] `readonly` на `name_raw` в форме OR и списке строк при `state != 'draft'`.
  - [ ] Серверная проверка в `object.request.line.write` (как у `qty_requested`).
  - [ ] Тесты: draft — можно, in_progress — UserError; регрессия OBR-015.
  - [ ] Закрыть TD-013, changelog после внедрения.
...
- **Статус**: Сделано в коде (`19.0.1.10.19`); шаблон на prod обновлён через MCP
...
- **Шаги выполнения**:
  ...
  - [ ] `-u object_request` на prod до `19.0.1.10.19` (шаблон уже изменён MCP).
- **Зависимости**: `object_request`, `purchase`, `mail.template`.

## Задача: OR — полная подпись в выпадашке товара/поставщика (OBR-041)
- **Статус**: Реализована локально (`19.0.1.10.17`); деплой на prod не выполнялся
...
- **Шаги выполнения**:
  ...
  - [ ] Деплой на prod: `-u object_request` → `19.0.1.10.17`.
