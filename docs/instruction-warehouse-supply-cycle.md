...
> **Смежная инструкция:** пополнение **базы** (Ос.ск, Расходники) — не через
> OR, а обычной закупкой: [`instruction-base-warehouse-replenish.md`](instruction-base-warehouse-replenish.md).
...
| **API / агент (MCP)** | Черновики OR/PO/picking; поиск остатков; подстановка данных из счёта; расчёт метража в плане закупки | Проведение без доработки validate (TD-003) |
...
3. Ввести **фактическое количество в метрах**; если счёт пришёл в кг / т / хлыстах, ассистент покажет пересчёт и предложит подставить метры (см. TD-002 и раздел 7).
4. **Проверить → Провести** (Validate).
...
Для труб AI Assistant показывает детерминированный пересчёт в метры в плане до подтверждения:
...
Запись в PO идёт только в метрах и только после подтверждения плана в чате или UI.
...
| Документ | Содержание |
|----------|------------|
| [technical-debt.md — TD-002](technical-debt.md#td-002-единицы-измерения-труб-метр--m-и-приход-на-склад-в-метрах-при-закупке-в-кгтоннах) | UoM метр/m, пересчёт кг→м в закупочном плане |
