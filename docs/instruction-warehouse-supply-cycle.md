...
Эта инструкция описывает **целевой рабочий процесс** пополнения складов объектов (ОбМ-*) в Odoo:
...
Документ также фиксирует **границы возможностей MCP/API** и известные ограничения (на примере счёта УТ-1132 и прихода ОбМ-4/IN/00001).
...
- Единая цепочка: **`object.request` (OR) → `purchase.order` (PO) / `stock.picking` (INT) → проведённый приход на `ОбМ-*/…`**
...
|-------|-----|------------|
| Амурком (Островского 234) | ОбМ-1 | Объект |
| Ломоносова 164 | ОбМ-2 | Объект |
| Ленина 231 | ОбМ-3 | Объект |
| **Б. Хмельницкого, 112** | **ОбМ-4** | Объект |
| Офис, Ос.ск, БАЗА, … | … | База / пополнение |

**Локация запасов объекта:** обычно `ОбМ-N/Наличие` или `ОбМ-N/Склад объекта` — смотреть в карточке склада (`lot_stock_id`).

### 5.3. Заказ поставщику — `purchase.order`

- **Поставщик:** `partner_id` (например ООО «ПроМеталл», id 19).
- **Склад назначения:** через `picking_type_id` типа **«ОбМ-N: Поступления»** — **не** «Офис: Receipts».
...
|-----|-------------------|--------|
| **Incoming** | ОбМ-N: Поступления | Закупка: Vendors → объект |
| **Internal** | ОбМ-N или база: Внутренние переводы | С базы на объект |
...
**Ошибка, которой избегать:** закупка на **Офис**, когда нужен **ОбМ-4** (как P00008 → Офис/IN вместо объекта).

---

### Шаг 3a. Пополнение с базы (внутреннее перемещение)

1. **Склад → Операции → Внутренние перемещения** (или тип **ОбМ-N: Внутренние переводы**).
2. **Из:** локация базы с остатком. **В:** `ОбМ-N/Наличие` (или аналог).
...
3. **Тип операции / склад:** выбрать склад **объекта** (например **Б. Хмельницкого, 112**) — Odoo подставит **ОбМ-4: Поступления**.
...
1. Из PO открыть **Приход** / **Receipt** (ОбМ-N/IN/…).
2. Проверить **назначение:** Vendors → **ОбМ-N/…**
...
|--------|-----------------|
| PO на «Офис», а нужен ОбМ-4 | В PO выбрать склад **Б. Хмельницкого, 112** |
