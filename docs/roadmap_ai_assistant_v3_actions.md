...
| Автоматический пересчёт **кг→м** при записи в строки | до закрытия TD-002 — только подсказка в чате, пересчёт делает пользователь | `docs/technical-debt.md` TD-002 |
...
5. Для труб — UoM «метр». Пересчёт кг/тонны в метры пользователь делает САМ;
   ты можешь предложить формулу из supply_cycle_context, но не записывай
   результат, если пользователь не подтвердил числа.
...
| Парсинг счёта в кг/тонн → метры с ошибкой → неверное `product_qty` | До TD-002: пересчёт **только** показывается в чате, в PO записывается значение, явно прозвучавшее в подтверждённом плане; пользователь подтверждает кнопкой. |
