...
| 12 | Wizard «есть на складе» (`stock_check_wizard`) | **Оставляем как предупреждение**; после «ОК» считается, что прораб ознакомлен (`stock_check_confirmed = True` сохраняется в требовании) |
...
- [ ] Зафиксировать в `docs/changelog.md` начало миграции структуры данных модуля `object_request`.
- [ ] Зафиксировать в `docs/project.md` обновлённую схему данных и поток выдачи (mermaid-диаграмма).
...
- [ ] Обновить `docs/functionalspecobjectrequest.md`: UI прораба/снабженца, новый wizard, массовые действия.
- [ ] Обновить `docs/changelog.md` (Изменено: схема склада; Добавлено: per-warehouse распределение, multi-picking wizard).
- [ ] Дополнить `docs/project.md`: mermaid-диаграмма потока «требование → расчёт → разбивка → выдача N pickings + PO».
- [ ] Обновить `docs/tasktracker.md`: добавить ссылку на этот файл, статус «В процессе».
...
- [ ] Документация (`changelog`, `project`, `datamodelspec`, `functionalspec`) обновлена.
