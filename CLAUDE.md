...
- Если правка переопределяет ядро Odoo (методы, письма, layout, xpath к
  стандартным views, портал) и её нужно будет учесть при переходе на новую
  версию — **сразу** добавить пункт `UPG-*` в `docs/odoo-upgrade-notes.md`
  (правило `.cursor/rules/odoo-upgrade-notes.mdc`)
- При работе с файлами вида `tasktracker*` / `tasktrecker*` отмечать выполненные задачи галочками (`[x]`) сразу после завершения соответствующего пункта.
